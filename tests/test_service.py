...
def test_register_version_is_idempotent() -> None:
    ...
    created = asyncio.run(service.register_version("registry.example.com/team/api", "1.2.3"))
    duplicate = asyncio.run(service.register_version("registry.example.com/team/api", "1.2.3"))
...
def test_register_rejects_build_metadata_only_variant() -> None:
    ...
    asyncio.run(service.register_version("registry.example.com/team/api", "1.2.3+build1"))

    with pytest.raises(EquivalentVersionExistsError):
        asyncio.run(service.register_version("registry.example.com/team/api", "1.2.3+build2"))
