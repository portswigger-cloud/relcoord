...
and relcoord edits the comment carrying that line instead of adding another.
`updated` in the response says which happened;
...
```json
{
  ...
  "validations": [
    {"output": "example-dev", "checks": ["structural"], "error": null,
     "passed": true, "digest": "sha256:…", "cached": false,
     "verdicts": [{"passed": true, "tool": "structural", "tool_version": "1",
                   "ruleset_digest": "sha256:…", "findings": []}]}
  ],
...
```toml
[validator]
url = "http://manifest-validator.platform.svc:8080"
# How long a scan may take. A cold kics run over a large tree is the slow case.
timeout-seconds = 900
...
what dev will run and promoting to `example-prod` scans what prod will run. The
same checks belong on both: a rule dev is not held to is a rule that fails for
the first time on the way to prod, where it is most expensive to act on, and
scanning dev first is what makes it an early warning instead. `checks` is per
...
because relcoord's own config is generated by the pipeline being validated: a
threshold moved here would need a `system` pull request, a regeneration, an Argo
sync and a pod restart to change.

What gets validated is the output's directory as it now stands, not the files the
commit happened to touch — the checks are about the manifests that will be
deployed. An output a commit leaves alone is therefore validated too, which costs
a request and no scan, because the validator keys its cache on the content digest
...
A finding is either accepted by manifest-validator, with the reason it was, or
it fails the verdict; the comment counts the first and tables the second.
Whether a finding is tolerable is that service's business, not relcoord's —
relcoord reads `passed` and never recomputes it.
