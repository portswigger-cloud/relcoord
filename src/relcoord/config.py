...
from relcoord.validator import (
    DEFAULT_TIMEOUT_SECONDS as DEFAULT_VALIDATION_TIMEOUT_SECONDS,
)
...
class ValidatorSettings:
    """Where manifest-validator is, and how long a scan may take.

    Nothing else belongs here. Which tools run, their versions, the rulesets and
    the pass/fail decision are that service's own configuration: relcoord's is
    generated by the pipeline being validated, so a threshold moved here would
    need a system PR, a regeneration, an Argo sync and a pod restart to change.
    """
    ...
    @classmethod
    def from_mapping(cls, data: dict[str, Any]) -> ValidatorSettings:
        url = data.get("url")
        if not isinstance(url, str) or not url.strip():
            raise ValueError("validator.url must be a non-empty string")
        timeout = data.get("timeout-seconds", cls.timeout_seconds)
        if isinstance(timeout, bool) or not isinstance(timeout, int | float):
            raise TypeError("validator.timeout-seconds must be a number")
        if timeout <= 0:
            raise ValueError("validator.timeout-seconds must be greater than zero")
        return cls(url=url, timeout_seconds=float(timeout))
...
class Settings:
    ...
    validator: ValidatorSettings | None = None
    ...
    def from_toml(cls, path: str | Path) -> Settings:
        ...
        if validator_data is not None and not isinstance(validator_data, dict):
            raise ValueError("validator must be a table")
        ...
        validator = (
            ValidatorSettings.from_mapping(validator_data)
            if validator_data is not None
            else None
        )
        _check_validated_outputs(outputs, validator)
...
def _check_validated_outputs(
    outputs: list[OutputSettings], validator: ValidatorSettings | None
) -> None:
    """Reject checks named for an output nothing will validate.
